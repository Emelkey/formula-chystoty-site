...
export function FloatingContactButtons() {
  return (
    <div className="fixed bottom-4 right-4 z-50 flex max-w-[calc(100vw-32px)] flex-col items-end gap-2">
      <TrackedLink href="/kontakty#contact-form" aria-label="Залишити заявку" className="inline-flex h-12 w-12 items-center justify-center gap-2 rounded-full bg-brand-green px-0 text-sm font-semibold text-white shadow-soft transition hover:bg-brand-hover focus-visible:focus-ring sm:h-auto sm:w-auto sm:px-5 sm:py-3" eventName="contact_form_open" eventCategory="lead" eventLabel="contact_form">
        <ClipboardCheck size={18} aria-hidden />
        <span className="sr-only sm:not-sr-only">Залишити заявку</span>
      </TrackedLink>
